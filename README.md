...
с использованием Docker layer cache и обновляет контейнер. Старый контейнер
работает во время сборки, поэтому недоступность ограничивается моментом его
замены. Скрипт сразу начинает опрашивать `/health` внутри контейнера и
завершается, как только приложение начинает отвечать.
