...
- Кабинет и офис берутся из `physicalDeliveryOfficeName`: например `БЯ-9\317` или `БЯ-7:13` превращаются в офис `БЯ9`/`БЯ7` и кабинет `317`/`13`.
...
- Внутренний API возвращает структурированные результаты без LDAP DN и других внутренних идентификаторов; фото отдаётся только ссылкой `photo_url`, без base64 в JSON.
...
`"has_more": true`. Поля `express_chat_url` и `photo_url` могут быть `null`.
`photo_url` возвращается только для записей с доступным `thumbnailPhoto`; API
не отдаёт фото в base64.
  ...
  "results": [
    {
      ...
      "photo_url": "http://127.0.0.1:8183/api/photos/example-token"
    }
  ],
  "has_more": false
}
...
запросы отклоняются до обращения к LDAP. `photo_url` содержит короткоживущий
opaque URL и не включает LDAP DN, ФИО, email или base64.
