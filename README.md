...
- День рождения берётся из `extensionAttribute4` и выводится как `📅 День рождения: 24 июня`, без года. Если дата совпадает с текущим днём, перед датой показывается `🎂`.
